...
import {
  ...
  IconRefresh,
  IconFilter,
  IconSearch, IconCash, IconUser,
  IconCategory,
  IconChartBar
} from '@tabler/icons-react'
import { getExpenseReport, exportFinancialReportCSV } from '@/server/actions/report-actions'
...
export function ExpenseReportContent() {
  ...
  useEffect(() => {
    fetchReportData()
  }, [dateRange, searchTerm, selectedUser, fetchReportData])
  ...
  const filteredAndSortedCategories = reportData?.breakdown
    .filter(category => {
      if (selectedCategory !== 'all' && category.categoryName !== selectedCategory) return false
      return true
    })
    .sort((a, b) => {
      ...
      <Card>
        <CardContent className="p-6">
          <div className="text-center text-muted-foreground">
            ...
            <p className="text-sm">No expense data found for the selected period.</p>
          </div>
        </CardContent>
      </Card>
    )
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div className="space-y-2">
          <h1 className="text-3xl font-bold tracking-tight">Expense Analysis Report</h1>
        ...
        <CardContent>
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-6">
            ...
            <div className="space-y-2">
              ...
              <Select value={sortBy} onValueChange={(value: 'amount' | 'date' | 'category') => setSortBy(value)}>
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{reportData.breakdown.length}</div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{reportData.userWiseExpenses.length}</div>
            <p className="text-xs text-muted-foreground">Team members with expenses</p>
          </CardContent>
        </Card>
      </div>
                  ...
                  <div key={index} className="border rounded-lg p-6 hover:shadow-md transition-shadow">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <div className="text-right">
                        <div className="text-2xl font-bold text-red-600">{formatCurrency(category.totalAmount)}</div>
                    ...
                    {category.monthlyTrend && category.monthlyTrend.length > 0 && (
                      <div className="mt-4 pt-4 border-t">
                        <h5 className="text-sm font-semibold mb-2">Monthly Trend</h5>
                        <div className="grid grid-cols-3 md:grid-cols-6 gap-2">
                          {category.monthlyTrend.slice(-6).map((trend, trendIndex) => (
                            <div key={trendIndex} className="text-center">
                              <div className="text-xs text-muted-foreground">{trend.month}</div>
                              <div className="text-sm font-medium">{formatCurrency(trend.amount)}</div>
                            </div>
                          ))}
                        </div>
                      </div>
            ...
            <CardContent>
              <div className="space-y-4">
                {reportData.userWiseExpenses
                  .sort((a, b) => sortOrder === 'desc' ? b.totalAmount - a.totalAmount : a.totalAmount - b.totalAmount)
                  .map((user, index) => (
                  <div key={index} className="border rounded-lg p-6 hover:shadow-md transition-shadow">
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground">
                            {user.transactionCount} expense{user.transactionCount !== 1 ? 's' : ''} submitted
                          </p>
                        </div>
                      </div>
                      <div className="text-right">
                        <div className="text-2xl font-bold text-red-600">{formatCurrency(user.totalAmount)}</div>
                        <div className="text-sm text-muted-foreground">
                          {reportData.totalExpenses > 0 ?
                            Math.round((user.totalAmount / reportData.totalExpenses) * 100) : 0}% of total
                        </div>
                      </div>
                    </div>
                      ...
                      <div
                        ...
                        style={{
                          width: `${reportData.totalExpenses > 0 ?
                            (user.totalAmount / reportData.totalExpenses) * 100 : 0}%`
                        }}
                      />
                    </div>
                  </div>
                ...
                <table className="w-full border-collapse">
                  <thead>
                    <tr className="border-b">
                      ...
                      <th className="text-center p-4 font-semibold">Total Amount</th>
                      <th className="text-center p-4 font-semibold">Transactions</th>
                      <th className="text-center p-4 font-semibold">Percentage</th>
                      <th className="text-center p-4 font-semibold">Avg per Transaction</th>
                    </tr>
                  </thead>
                  <tbody>
                    {filteredAndSortedCategories?.map((category, index) => (
                        ...
                        <td className="p-4">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <div className="text-sm text-muted-foreground">ID: {category.categoryId}</div>
                            </div>
                          </div>
                        </td>
                        <td className="text-center p-4 font-medium text-red-600">{formatCurrency(category.totalAmount)}</td>
                        <td className="text-center p-4 font-medium">{category.transactionCount}</td>
