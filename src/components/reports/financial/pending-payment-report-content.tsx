...
import {
  ...
  IconTrendingUp,
  IconClock,
  IconExclamationMark, IconArrowDown,
  IconTarget
} from '@tabler/icons-react'
import { getPendingPaymentReport, exportFinancialReportCSV } from '@/server/actions/report-actions'
...
export function PendingPaymentReportContent() {
  const [reportData, setReportData] = useState<PendingPaymentReport | null>(null)
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)
  const [searchTerm, setSearchTerm] = useState('')
  const [dateRange, setDateRange] = useState<DateRangeFilter | undefined>()
  const [selectedPriority, setSelectedPriority] = useState<string>('all')
  const [sortBy, setSortBy] = useState<'amount' | 'daysOverdue' | 'name' | 'course'>('amount')
  ...
  const filteredAndSortedStudents = reportData?.students
    .filter(student => {
      if (selectedPriority !== 'all' && student.priority !== selectedPriority) return false
      return true
    })
    .sort((a, b) => {
      ...
      <Card>
        <CardContent className="p-6">
          <div className="text-center text-muted-foreground">
            ...
            <p className="text-sm">No pending payment data found for the selected period.</p>
          </div>
        </CardContent>
      </Card>
    )
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div className="space-y-2">
          <h1 className="text-3xl font-bold tracking-tight">Pending Payment Report</h1>
        ...
        <CardContent>
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-5">
            ...
            <div className="space-y-2">
              ...
              <Select value={sortBy} onValueChange={(value: 'amount' | 'daysOverdue' | 'name' | 'course') => setSortBy(value)}>
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-red-600">{formatCurrency(reportData.totalOutstanding)}</div>
            <p className="text-xs text-muted-foreground">Amount pending collection</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">Students with outstanding dues</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">Urgent collections needed</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {formatCurrency(reportData.totalStudents > 0 ? reportData.totalOutstanding / reportData.totalStudents : 0)}
            </div>
        ...
        <TabsContent value="outstanding" className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Real-time tracking of students with pending payments and priority management
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                {filteredAndSortedStudents?.slice(0, 10).map((student, index) => (
                  <div key={index} className="border rounded-lg p-6 hover:shadow-md transition-shadow">
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground">
                            {student.course} • {student.daysOverdue} days overdue
                          </p>
                        </div>
                      </div>
                      <div className="text-right">
                        <div className="text-2xl font-bold text-red-600">{formatCurrency(student.outstandingAmount)}</div>
                        <Badge variant="outline" className={`mt-1 ${getPriorityColor(student.priority)}`}>
                          {getPriorityIcon(student.priority)}
                          <span className="ml-1 capitalize">{student.priority} Priority</span>
                        </Badge>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-3 gap-4 text-center">
                      ...
                      <div className="p-3 rounded-lg bg-orange-50 border border-orange-200">
                        ...
                        <p className="text-sm text-orange-700">Days Overdue</p>
                      </div>
                      <div className="p-3 rounded-lg bg-blue-50 border border-blue-200">
                        <div className="text-lg font-bold text-blue-600">
                          {student.lastPaymentDate ? formatDate(student.lastPaymentDate) : 'No Payment'}
                        </div>
        ...
        <TabsContent value="aging" className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Breakdown of outstanding payments by age categories for collection prioritization
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                  ...
                  <div key={index} className="border rounded-lg p-6 hover:shadow-md transition-shadow">
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <h4 className="text-lg font-semibold">{aging.range}</h4>
                          <p className="text-sm text-muted-foreground">
                            {aging.count} student{aging.count !== 1 ? 's' : ''} in this category
                          </p>
                        </div>
                      </div>
                      <div className="text-right">
                        <div className="text-2xl font-bold text-orange-600">{formatCurrency(aging.amount)}</div>
                        <Badge variant="outline" className="mt-1">
                          {((aging.amount / reportData.totalOutstanding) * 100).toFixed(1)}% of total
                        </Badge>
                      </div>
                    </div>
                    ...
                    <div className="w-full bg-gray-200 rounded-full h-2 mb-4">
                      <div
                        ...
                        style={{ width: `${(aging.amount / reportData.totalOutstanding) * 100}%` }}
                      />
                    </div>

                    <div className="grid grid-cols-3 gap-4 text-center">
                      <div className="p-3 rounded-lg bg-orange-50 border border-orange-200">
                        <div className="text-lg font-bold text-orange-600">{formatCurrency(aging.amount)}</div>
                        <p className="text-sm text-orange-700">Total Amount</p>
                      </div>
                      <div className="p-3 rounded-lg bg-blue-50 border border-blue-200">
                        <div className="text-lg font-bold text-blue-600">{aging.count}</div>
                        <p className="text-sm text-blue-700">Students</p>
                      </div>
                      <div className="p-3 rounded-lg bg-purple-50 border border-purple-200">
                        <div className="text-lg font-bold text-purple-600">
                          {aging.count > 0 ? formatCurrency(aging.amount / aging.count) : formatCurrency(0)}
                        </div>
                        <p className="text-sm text-purple-700">Avg per Student</p>
                      </div>
                    </div>
                  </div>
                ))}
        ...
        <TabsContent value="targets" className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Strategic collection targets to improve cash flow and reduce outstanding dues
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                <div className="border rounded-lg p-6 hover:shadow-md transition-shadow">
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h4 className="text-lg font-semibold">This Week Target</h4>
                        <p className="text-sm text-muted-foreground">
                ...
                <div className="border rounded-lg p-6 hover:shadow-md transition-shadow">
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h4 className="text-lg font-semibold">This Month Target</h4>
                        <p className="text-sm text-muted-foreground">
                ...
                <div className="border rounded-lg p-6 hover:shadow-md transition-shadow">
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h4 className="text-lg font-semibold">This Quarter Target</h4>
                        <p className="text-sm text-muted-foreground">
                    ...
                    <div className="text-right">
                      <div className="text-2xl font-bold text-purple-600">
                        {formatCurrency(reportData.collectionTargets.thisQuarter)}
                      </div>
                      <Badge variant="outline" className="mt-1">
                        Quarterly Goal
                      </Badge>
        ...
        <TabsContent value="detailed-view" className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Complete list of all students with outstanding payments and detailed information
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="overflow-x-auto">
                    ...
                    {filteredAndSortedStudents?.map((student, index) => (
                      <tr key={index} className="border-b hover:bg-muted/50">
                        ...
                        <td className="p-4">
                          ...
                          <div className="text-sm text-muted-foreground">Course enrollment</div>
                        </td>
                        ...
                        <td className="text-center p-4">
                          <Badge variant="outline" className={
                            student.daysOverdue > 30 ? 'border-red-200 text-red-800' :
                            student.daysOverdue > 7 ? 'border-yellow-200 text-yellow-800' :
                ...
                {(!filteredAndSortedStudents || filteredAndSortedStudents.length === 0) && (
                  <div className="text-center py-8 text-muted-foreground">
                    ...
                    <p className="text-sm">No students with outstanding payments found for the selected criteria.</p>
                  </div>
                )}
              </div>
            </CardContent>
